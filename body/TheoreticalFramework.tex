...
$\mathbf{f}_k$~\ref{Brunelli}.

Template matching is often refers to matching performed using the template
descriptor as outlined in Subsection~\ref{literature_review_template}. However
it can be extended to the matching derivative descriptors of the initial
template.
This paper limits itself to the case of simple template matching based on the template descriptor
defined by and extraction of pixels over a particular region of interest in a
particular frame.

\subsection{Similarity measure}
The degree of similarity between a specific $T$ and a target region
within a particular frame $\mathbf{f}_k$ is determined by the use of a similarity
measure. The literature lists several similarity measures, the most widely
adopted of which is the pixel-wise \textit{sum of square differences} (ssd)~\ref{Brunelli}.
...
the sum or square differences similarity measure is the location of the object
...
the remainder of the image sequence $\mathbf{f}_k\ldots\mathbf{f_k+1}$.

The above tracker is based on the assumption that an particular target object
...
The second variant of TMT, termed the \textit{adaptive template tracker} (ATT),
attempts to address this issue by amending the assumption in the STT. The
underlying assumption in the ATT is that target objects only maintain the same
appearance for a (small) set of $K$-adjacent frames $\mathbf{f}_k \ldots
\mathbf{f}_{k+K}$ within a particular image sequence. To account for the change
beyond the $K$-frame interval, the ATT updates the model to be matched before
the target object appearance can vary enough for the similarity measure to fall
below the set threshold $\tau$.
...
between consecutive frames $\mathbf{f}_k$ and $\mathbf{f}_{k+1}$}{design_mean_shift_tracking_algorithm}
