...
\subsection{Kernel $\kappa(x)$: Epanechnikov Kernel}
...
\subsection{Histogram development}
Computing the histogram then occurs according to the following equation:
\begin{equation}
...
\subsection{Similarity Measure $\rho$: Battacharryya Coefficient}
...
term we maximise the Bhattacharryya Coefficient which corresponds to locating
our object in frame k+1. 

The calculation is detailed in \ref{AppendixA}. Of interest is the result
...
\Figure[width=0.8\columnwidth]{Flow Chart of Mean Shift Tracking Algorithm
