...
The image sequences are mainly taken from the VOT2017 challenge dataset
\cite{VOT_TPAMI}, and the famous Hamburg taxi sequence is also used as an
initial experiment for the template matching trackers.

The analysis of the algorithms is both quantitative and qualitative. The
quantitative analysis is based on the tracker performance per the centroid error metric
defined in Section~\ref{methodology_data_analysis}. 
...
Figure~\ref{fig:results/mean_shift_tracker/metric.png}. From the graph it can be
seen that the mean shift tracker actually performs better under occlusion the in
...
In Figure~\ref{fig:mean_shift_partial_occusion} Our target, the yellow fish
